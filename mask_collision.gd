...
#func _ready() -> void:
	...
	#await RenderingServer.frame_post_draw 
#
	#var image := mask_viewer.get_texture().get_image()
...
#
#
#func generate_collision_region_old(bitmap: BitMap, region_rect: Rect2) -> void:
...
#func _create_regions() -> void:
	#for x_index: int in ceil(mask_viewer.get_texture().get_size().x / rect_size.x):
		#for y_index: int in ceil(mask_viewer.get_texture().get_size().y / rect_size.y):
			#var x_start: float = x_index * rect_size.x
			#var y_start: float = y_index * rect_size.y
			#var start_pos := Vector2(x_start, y_start)
			#
			#var x_size: float = min(rect_size.x, mask_viewer.get_texture().get_size().x - x_start)
			#var y_size: float = min(rect_size.y, mask_viewer.get_texture().get_size().y - y_start)
...
#func _on_layer_mask_img_updated(updated_region_rect: Rect2i, new_image: Image) -> void:
	...
	#for x in range(x_start, x_end):
		#for y in range(y_start, y_end):
			#mask_bitmap.set_bit(x, y, mask_viewer.get_texture().get_image().get_pixel(x,y).a > 0)
	#
